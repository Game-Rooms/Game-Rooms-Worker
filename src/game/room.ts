...
import { handleMessage } from "../engine/router";
import { hostWelcomeResult, playerWelcomeResult, clientConnectedResult } from "./messages";
import type { JoinInfo, RoomAttachment } from "../engine/types";
...
export class Room extends DurableObject<Env> {
	...
	private async acceptHost(ws: WebSocket): Promise<void> {
		...
		ws.serializeAttachment({ id: HOST_ID, role: "host" } satisfies RoomAttachment);
		const connection = new Connection(HOST_ID, "host", ws);
	...
	private async acceptPlayer(ws: WebSocket, name: string): Promise<boolean> {
		...
		if (state.roomLocked || state.isFull) return false;

		const id = state.nextPlayerId();
		const username = name || `PLAYER ${id - HOST_ID}`;
		...
		if (host) host.send(state.bumpHostPc(), "client/connected", clientConnectedResult(connection));
